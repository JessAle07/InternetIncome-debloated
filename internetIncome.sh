...
start_containers() {
  ...
  if [[ $CASTAR_SDK_KEY ]]; then
    ...
    docker_parameters=($LOGS_PARAM $DNS_VOLUME $MAX_MEMORY_PARAM $MEMORY_RESERVATION_PARAM $MEMORY_SWAP_PARAM $CPU_PARAM $NETWORK_TUN -e KEY=$CASTAR_SDK_KEY techroy23/docker-castarsdk)
