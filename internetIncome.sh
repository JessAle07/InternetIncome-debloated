...
back_up_folders=($bitping_data_folder $urnetwork_data_folder $traffmonetizer_data_folder $mysterium_data_folder)
...
start_containers() {
  ...
  if [ "$UPROCK" = true ]; then
    if [ "$container_pulled" = false ]; then
      sudo docker pull --platform=linux/amd64 ghcr.io/bhavishyadahiya/uprock-docker/uprock:latest
    fi
    if [[ ! $proxy ]] || [ "$vpn_enabled" = false ]; then
      uprock_first_port=$(check_open_ports $uprock_first_port)
      if ! expr "$uprock_first_port" : '[[:digit:]]*$' >/dev/null; then
    ...
    docker_parameters=(--platform=linux/amd64 $LOGS_PARAM $DNS_VOLUME $MAX_MEMORY_PARAM $MEMORY_RESERVATION_PARAM $MEMORY_SWAP_PARAM $CPU_PARAM $NETWORK_TUN $uprock_container_port -e VNC_PORT=5722 -e WEBSOCKIFY_PORT=5111 -e VNC_PASSWORD="internetincome" ghcr.io/bhavishyadahiya/uprock-docker/uprock:latest)
  ...
  if [[ $PACKET_SDK_APP_KEY ]]; then
    ...
    docker_parameters=($LOGS_PARAM $DNS_VOLUME $MAX_MEMORY_PARAM $MEMORY_RESERVATION_PARAM $MEMORY_SWAP_PARAM $CPU_PARAM $NETWORK_TUN techroy23/docker-packetsdk:latest -e APPKEY=$PACKET_SDK_APP_KEY)
...
# Starting Honeygain container
  ...
  if [[ $HONEYGAIN_EMAIL && $HONEYGAIN_PASSWORD && "$HONEYGAIN_POT" = true ]]; then
    if [ "$container_pulled" = false ]; then
      sudo docker pull ghcr.io/xterna/honeygain-pot:latest
      docker_parameters=($LOGS_PARAM $DNS_VOLUME $MAX_MEMORY_PARAM $MEMORY_RESERVATION_PARAM $MEMORY_SWAP_PARAM $CPU_PARAM $NETWORK_TUN -e EMAIL=$HONEYGAIN_EMAIL -e PASSWORD=$HONEYGAIN_PASSWORD ghcr.io/xterna/honeygain-pot:latest)
  ...
  if [ "$EARNAPP" = true ]; then
    ...
    if CONTAINER_ID=$(sudo docker run -d --health-interval=24h --name earnapp$UNIQUE_ID$i $LOGS_PARAM $DNS_VOLUME $RESTART_POLICY $NETWORK_TUN -v $PWD/$earnapp_data_folder/data$i:/etc/earnapp -e EARNAPP_UUID=$uuid madereddy/earnapp:latest); then
      echo "$CONTAINER_ID" | tee -a $containers_file
